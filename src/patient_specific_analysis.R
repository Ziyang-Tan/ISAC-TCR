...
virus_specific_clone_id <- obj[[]] %>%
  filter(virus_specific == "virus_specific") %>%
  select(clone_id) %>%
  distinct() %>%
  unlist()
...
df <- df %>% mutate(
  trend = case_when(
    clone_id %in% virus_specific_clone_id ~ "virus_specific",
...
obj_sub <- subset(obj, subset = patient_id == patient &
  ...
  cell_type == sub_name)
tmp <- obj_sub[[]] %>%
  left_join(df %>% select(clone_id, trend) %>% distinct(), by = "clone_id") %>%
  select(trend)
