...
# dataset Wu ------------------------------------
# Wu, T.D., Madireddi, S., de Almeida, P.E. et al.
# Peripheral T cell expansion predicts tumour infiltration and clinical response. Nature 579, 274–278 (2020).
...
volcano_tmp <- function(df) {
  ...
  res <- left_join(res_fc, res_p, by = "gene")
  ...
  EnhancedVolcano(res,
    x = "logFC", y = "p.adj", lab = res$gene,
    pCutoff = 1e-02, FCcutoff = NA, drawConnectors = TRUE, arrowheads = FALSE,
    selectLab = lab,
    xlim = c(-1, 1),
    col = c("grey", "black", "red", "orange"),
    subtitle = paste0(
      "expanded(", table(df$expansion)["expanded"],
...
df_wu %>%
  filter(tumor_type == "Renal") %>%
  volcano_tmp() + ggtitle("memory CD8 T cells in Blood (Renal)")
...
df_wu %>%
  ...
  volcano_tmp() + ggtitle("memory CD8 T cells in Blood (Lung)")
...
EnhancedVolcano(res,
  ...
  arrowheads = F,
  subtitle = paste0(
    "expanded(", table(obj_sub[[]]$Clone.Status)["Clonal"],
...
obj_Zhang_sub <- subset(obj_Zhang,
  subset = Clonal.status != "NA" &
    ...
    ) # exclude CD8 T cells with LEF1 expression
)
obj_Zhang_sub[[]]
Idents(obj_Zhang_sub) <- "Clonal.status"
res <- FindMarkers(obj_Zhang_sub,
  ident.1 = "Clonal", ident.2 = "NoClonal",
  logfc.threshold = 0, min.pct = 0.5
)
top_p_genes <- res %>%
...
EnhancedVolcano(res,
  ...
  arrowheads = F,
  subtitle = paste0(
    "expanded(", table(obj_sub[[]]$Clonal.status)["Clonal"],
...
df <- rbind(
  df_wu %>% group_by(patient, expansion, tumor_type) %>% tally() %>% group_by(patient) %>% mutate(sum = sum(n), fraction = n / sum),
...
ggplot(df, aes(x = factor(tumor_type, levels = unique(df$tumor_type)), y = fraction, color = group)) +
  ...
  xlab("tumor_type") +
  ylab("Fraction of expanded cells in blood memory CD8T cells")
